...
```bash
curl -fsSL https://raw.githubusercontent.com/NVIDIA/NeMo-Relay/main/install.sh | NEMO_RELAY_VERSION=0.9.1-rc.1 sh
```
...
```powershell
$env:NEMO_RELAY_VERSION = '0.9.1-rc.1'
irm https://raw.githubusercontent.com/NVIDIA/NeMo-Relay/main/install.ps1 | iex
```
...
```bash
cargo add nemo-relay@0.9.1-rc.1
cargo add nemo-relay-adaptive@0.9.1-rc.1
```
...
```bash
openclaw plugins install npm:nemo-relay-openclaw@0.9.1-rc.1
...
```bash
uv add "nemo-relay[langchain,langgraph,deepagents]==0.9.1-rc.1"
```
