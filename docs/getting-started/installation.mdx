...
This wheel installs `nemo-plugin-host` beside the `nemo-relay` executable. The
CLI starts that host to run native plugins outside its own process, and it looks
for it in the directory holding the executable that started it, so the two are
installed together rather than separately.
...
```bash
curl -fsSL https://raw.githubusercontent.com/NVIDIA/NeMo-Relay/main/install.sh | NEMO_RELAY_VERSION=0.9.1-rc.4 sh
```
...
```powershell
$env:NEMO_RELAY_VERSION = '0.9.1-rc.4'
irm https://raw.githubusercontent.com/NVIDIA/NeMo-Relay/main/install.ps1 | iex
```
...
```bash
cargo add nemo-relay@0.9.1-rc.4
cargo add nemo-relay-adaptive@0.9.1-rc.4
```
...
```bash
openclaw plugins install npm:nemo-relay-openclaw@0.9.1-rc.4
...
```bash
uv add "nemo-relay[langchain,langgraph,deepagents]==0.9.1-rc.4"
```
