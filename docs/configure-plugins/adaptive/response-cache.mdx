...
Use `principal` or `tenant` only when the active scope carries the matching
`nemo_relay.identity` metadata; missing identity fails closed with an
...
| `share_scope` | `"session"` | Identity partition: `session`, `principal`, `tenant`, or explicit `global`. Principal and tenant modes fail closed when active identity metadata is absent. |
