...
description: "Configure the Adaptive component, including state, telemetry, hints, ACG, provider admission, and response caching."
position: 2
---
...
| `provider_admission` | Always-on provider concurrency, pending-work, and admission-timeout limits. These apply whether response caching is enabled or not. |
...
- Tune `provider_admission` independently when provider capacity or pending work
  must be bounded without enabling response caching.
...
[components.config.provider_admission]
