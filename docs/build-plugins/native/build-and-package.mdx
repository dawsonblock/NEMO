...
The repository example builds a `cdylib` against the current 0.9.1-rc.1 SDK and packages it
...
dynamic boundary. The dependencies use the current 0.9.1-rc.1 public SDK rather than a Git
...
plugin depends on the published `nemo-relay-plugin = "0.9.1-rc.1"` package without `path`.
...
[dependencies]
futures = "0.3"
nemo-relay-plugin = { version = "0.9.1-rc.1", path = "../../crates/plugin" }
