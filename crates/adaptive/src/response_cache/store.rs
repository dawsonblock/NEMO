...
static CACHE_WRITES: LazyLock<CacheWriteState> = LazyLock::new(|| CacheWriteState {
...
impl CacheWritePermit {
    fn acquire() -> Self {
        CACHE_WRITES.pending.fetch_add(1, Ordering::AcqRel);
        Self
    }
}
...
impl Drop for CacheWritePermit {
    fn drop(&mut self) {
        if CACHE_WRITES.pending.fetch_sub(1, Ordering::AcqRel) == 1 {
            CACHE_WRITES.idle.notify_waiters();
        }
    }
}

/// Store one entry while registering it with the process-wide drain barrier.
pub(crate) async fn tracked_set(
    ...
    let _permit = CacheWritePermit::acquire();
    store.set(key, entry, ttl).await
}

/// Wait until all cache writes already in progress have completed.
pub async fn wait_for_cache_idle() {
    loop {
        let notified = CACHE_WRITES.idle.notified();
        if CACHE_WRITES.pending.load(Ordering::Acquire) == 0 {
            return;
        }
        notified.await;
    }
...
/// Flush process-local cache writes before graceful shutdown.
pub async fn flush_cache_writes() {
    wait_for_cache_idle().await;
}
