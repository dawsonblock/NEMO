...
/// Point-in-time health snapshot for the bounded native subscriber queue.
#[napi(object)]
pub struct SubscriberDispatcherMetrics {
    pub relay_event_queue_depth: u32,
    pub relay_event_queue_capacity: u32,
    pub relay_event_queue_high_watermark: u32,
    pub relay_events_enqueued_total: u32,
    pub relay_events_dropped_total: u32,
    pub relay_events_backpressured_total: u32,
}

fn metric_count(value: usize) -> u32 {
    u32::try_from(value).unwrap_or(u32::MAX)
}

/// Return bounded subscriber queue depth, capacity, and cumulative counters.
#[napi]
pub fn subscriber_dispatcher_metrics() -> SubscriberDispatcherMetrics {
    let metrics = nemo_relay::api::runtime::subscriber_dispatcher_metrics();
    SubscriberDispatcherMetrics {
        relay_event_queue_depth: metric_count(metrics.queue_depth),
        relay_event_queue_capacity: metric_count(metrics.queue_capacity),
        relay_event_queue_high_watermark: metric_count(metrics.queue_high_watermark),
        relay_events_enqueued_total: metric_count(metrics.events_enqueued_total),
        relay_events_dropped_total: metric_count(metrics.events_dropped_total),
        relay_events_backpressured_total: metric_count(metrics.events_backpressured_total),
    }
}
