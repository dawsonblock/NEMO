...
describe('LLM guardrails', () => {
  ...
  it('surfaces JavaScript finalizer exceptions as terminal stream errors', async () => {
    const stream = await llmStreamCallExecute(
      'streaming_finalizer_error',
      makeNative(),
      (wrapper) => {
        lib.pushStreamChunk(wrapper.__nemo_relay_stream_id, { token: 'visible' });
        lib.endStream(wrapper.__nemo_relay_stream_id);
      },
      null,
      () => {
    ...
    assert.deepEqual(await stream.next(), { token: 'visible' });
    await assert.rejects(() => stream.next(), /finalizer exploded/i);
  });
