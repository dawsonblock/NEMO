...
it('exposes bounded subscriber queue metrics', () => {
  const metrics = lib.subscriberDispatcherMetrics();
  assert.ok(metrics.relayEventQueueCapacity > 0);
  assert.ok(metrics.relayEventQueueDepth >= 0);
  assert.ok(metrics.relayEventsDroppedTotal >= 0);
});
