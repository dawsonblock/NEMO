...
pub struct LlmStreamWrapper {
    ...
    finalizer: Option<Box<dyn FnOnce() -> Result<Json> + Send>>,
...
impl LlmStreamWrapper {
    ...
    pub(crate) fn new_managed(
        ...
        handle: LlmHandle,
        collector: Box<dyn FnMut(Json) -> Result<()> + Send>,
        finalizer: Box<dyn FnOnce() -> Result<Json> + Send>,
        metadata: Option<Json>,
        response_codec: Option<Arc<dyn LlmResponseCodec>>,
        subscribers: Vec<EventSubscriberFn>,
    ) -> Self {
    ...
    fn finish_with_error(&mut self, error: &FlowError) {
        ...
        let _ = self.emit_end_event(metadata, StreamTermination::Failed);
    }
    ...
    fn emit_end_event(
        ...
        let (aggregated, metadata, finalizer_error) = match aggregated {
            Ok(aggregated) => (aggregated, metadata, None),
            Err(error) => {
                let metadata = metadata_with_otel_error(metadata, &error);
                (Json::Null, metadata, Some(error))
            }
        };
...
impl Stream for LlmStreamWrapper {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match Pin::new(&mut this.inner).poll_next(cx) {
            ...
            Poll::Ready(None) => {
                if let Err(error) = this.finish_cleanly() {
                    this.terminal_result = Some(Err(error));
                }
                self.poll_next(cx)
            }
...
impl LlmStreamInner for LlmStreamWrapper {
    fn close(self: Pin<&mut Self>) -> Pin<Box<dyn Future<Output = Result<()>> + Send + '_>> {
        ...
        Box::pin(async move {
            ...
            let finalizer_result = this.finish();
            let result = result.and(finalizer_result);
