...
pub(super) fn validate_dynamic_plugin_relay_compatibility(
    ...
    // Development/pre-release hosts should still satisfy the compatibility
    // range for their eventual release line. Semver intentionally excludes a
    // prerelease from a plain `>=0.8.0` range, but rejecting every worker while
    // validating the 0.9.1 release candidate would make the development line
    // unusable. Keep exact prerelease constraints strict; only fall back to the
    // normalized release identity for ordinary ranges.
    let normalized_host = Version {
        major: host_version.major,
        minor: host_version.minor,
        patch: host_version.patch,
        pre: semver::Prerelease::EMPTY,
        build: semver::BuildMetadata::EMPTY,
    };
    if !requirement.matches(&host_version)
        && (!host_version.pre.is_empty() && !requirement.matches(&normalized_host))
    {
        return Err(PluginError::InvalidConfig(format!(
