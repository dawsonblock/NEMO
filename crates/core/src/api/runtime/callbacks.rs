...
/// Fallible finalizer used by bindings whose callback runtime can fail while
/// producing the aggregate streaming response.
///
/// Unlike [`LlmFinalizerFn`], callback failures remain distinct from a valid
/// JSON `null` response and terminate the managed stream with an error.
pub type LlmFallibleFinalizerFn = Box<dyn FnOnce() -> Result<Json> + Send>;
