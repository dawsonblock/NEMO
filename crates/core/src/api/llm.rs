...
pub async fn llm_stream_call_execute(params: LlmStreamCallExecuteParams) -> Result<LlmJsonStream> {
    llm_stream_call_execute_inner(params, None).await
}

/// Execute a managed streaming LLM call with a fallible finalizer.
///
/// This entry point exists for language bindings whose callback transport can
/// fail independently of the provider stream. A finalizer error is surfaced as
/// the terminal stream item and recorded on the LLM end event.
#[doc(hidden)]
pub async fn llm_stream_call_execute_with_fallible_finalizer(
    params: LlmStreamCallExecuteParams,
    fallible_finalizer: crate::api::runtime::LlmFallibleFinalizerFn,
) -> Result<LlmJsonStream> {
    llm_stream_call_execute_inner(params, Some(fallible_finalizer)).await
}

async fn llm_stream_call_execute_inner(
    params: LlmStreamCallExecuteParams,
    fallible_finalizer: Option<crate::api::runtime::LlmFallibleFinalizerFn>,
) -> Result<LlmJsonStream> {
    ...
    let finalizer: crate::api::runtime::LlmFallibleFinalizerFn =
        fallible_finalizer.unwrap_or_else(|| Box::new(move || Ok(finalizer())));
    ensure_runtime_owner()?;
    {
        let (entries, subscribers, parent_uuid, guardrail_metadata) = {
            let scope_stack = current_scope_stack();
