...
use super::{
    EventSubscriberFn, SubscriberDelivery, SubscriberDispatcherMetrics, publication_context,
...
#[test]
fn dispatcher_metrics_use_stable_relay_metric_names() {
    let value = serde_json::to_value(SubscriberDispatcherMetrics::default()).unwrap();
    assert!(value.get("relay_event_queue_depth").is_some());
    assert!(value.get("relay_event_queue_high_watermark").is_some());
    assert!(value.get("relay_events_dropped_total").is_some());
    assert!(value.get("relay_events_backpressured_total").is_some());
}
